from g_model import g_net
from simple_model import CleanU_Net
from dataset import *
import torch
...
if __name__ == "__main__":
    ...
    SEM_train = SEMDataTrain(
        '../data/train/images', '../data/train/masks', in_size=64, out_size=64)

    # TO DO: finish test data loading
    SEM_test = SEMDataTest(
        '../data/test/images/', in_size=64 ,out_size=64)
    SEM_val = SEMDataVal(
        '../data/val/images', '../data/val/masks', in_size=64, out_size=64)
    # Dataset end
    ...
    SEM_val_load = \
        torch.utils.data.DataLoader(dataset=SEM_val, batch_size=1, shuffle=True)
