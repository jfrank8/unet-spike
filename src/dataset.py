...
class SEMDataTrain(Dataset):
  ...
  #      img_as_img.show()
        ...
        #shrik it a little first
        #breakpoint()
        img_as_np = transform.resize(img_as_np, (192, 192), preserve_range=True)
        # Crop the image
...
#        img1 = Image.fromarray(img_as_np)
 #       img1.show()
        ...
        #breakpoint()
        #shrink it a 'lil first
        msk_as_np = transform.resize(msk_as_np, (192, 192), preserve_range=True)
        # Crop the mask
        #breakpoint()
...
class SEMDataVal(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #do a little ensmallening
        img_as_np = transform.resize(img_as_np, (192, 192), preserve_range=True)

        crop_num = math.ceil( np.shape(img_as_np)[0] / (self.in_size) )
        #breakpoint()
        img_as_np = multi_cropping(img_as_np,
                                   crop_size=self.in_size,
                                   crop_num1=crop_num, crop_num2=crop_num)
        ...
        crop_num = math.ceil( np.shape(msk_as_np)[1] / (self.out_size) )
        #
