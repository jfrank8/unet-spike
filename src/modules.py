...
def train_model(model, data_train, criterion, optimizer, device = 'cpu'):
    """Train the model and report validation error with training error
    Args:
        model: the model to be traine, device = 'cpud
    ...
    for batch, (images, masks) in enumerate(data_train):
        ...
        outputs = F.interpolate(outputs,64)
        #print('outputs size: '+str( outputs.size()))
        #print('mask size: '+str(masks.size()))
        #print(outputs.unique())
...
def get_loss_train(model, data_train, criterion, device = 'cpu'):
    """
...
def validate_model(model, data_val, criterion, epoch, make_prediction=True, save_folder_name='prediction', device = 'cpu'):
    """
        Validation run
    """
    ...
    for batch, (images_v, masks_v, original_msk) in enumerate(data_val):
        ...
        for index in range(images_v.size()[1]):
            with torch.no_grad():
                ...
                mask_v = Variable(masks_v[:, index, :, :].squeeze(1).to(device))
                # print(image_v.shape, mask_v.shape)
                output_v = model(image_v)
                output_v = F.interpolate(output_v, 64)
                #breakpoint()
...
def test_model(model_path, data_test, epoch, save_folder_name='prediction', device = 'cpu'):
    """
        Test run
    """
    ...
    for batch, (images_t) in enumerate(data_test):
        ...
        for index in range(images_t.size()[1]):
            with torch.no_grad():
                image_t = Variable(images_t[:, index, :, :].unsqueeze(0).to(device))
                # print(image_v.shape, mask_v.shape)
